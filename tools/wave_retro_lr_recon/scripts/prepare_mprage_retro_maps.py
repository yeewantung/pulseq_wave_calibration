#!/usr/bin/env python3
"""Resample one native BART ecalib map set onto all MPRAGE LR PE grids."""

from __future__ import annotations

import argparse
import sys
from pathlib import Path
from typing import Sequence

TOOL_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(TOOL_ROOT))

from wave_retro_lr.mprage import prepare_retro_sensitivity_maps  # noqa: E402


def main(argv: Sequence[str] | None = None) -> int:
    """Parse the output root and derive all retrospective CSM grids.

    Args:
        argv: Optional argument vector; ``None`` reads the process arguments.

    Returns:
        Zero after sensitivity-map preparation succeeds.
    """
    args = _parser().parse_args(argv)
    prepare_retro_sensitivity_maps(args.output)
    return 0


def _parser() -> argparse.ArgumentParser:
    """Build the CSM-resampling command-line interface.

    Returns:
        Configured parser accepting one dataset output root.
    """
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "output", type=Path, help="Dataset output root containing normal and retro folders."
    )
    return parser


if __name__ == "__main__":
    try:
        raise SystemExit(main())
    except (FileExistsError, FileNotFoundError, RuntimeError, ValueError) as exc:
        print(f"Error: {exc}", file=sys.stderr)
        raise SystemExit(2) from exc
