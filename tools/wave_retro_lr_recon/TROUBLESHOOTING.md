...
the shaded region is the user-supplied half-open fit interval `[min, max)`;
...
The default remains nine-sample smoothing. The optional sine-plus-line model
requires explicit `--psf-fit-kx-min` and `--psf-fit-kx-max` values. Use the same
three PSF options accepted by either MPRAGE sample script, for example:
