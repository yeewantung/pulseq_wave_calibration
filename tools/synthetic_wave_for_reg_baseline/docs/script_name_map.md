...
| `scripts/run_no_wave_r3x1_grappa.py` | Presentation comparison | Applies the same retrospective R3x1-plus-ACS mask and reuses the accepted local joint-coil 5x5x5 GRAPPA implementation, with resumable checkpoints, magnitude/phase export, and exact-grid direct-FFT metrics. |
...
| `scripts/evaluate_no_wave_r3x1_sweep.py` | Presentation evaluation | Validates no-Wave R3x1 case metrics and plots Wavelet metrics over lambda with CG-SENSE and GRAPPA controls, reporting descriptive leaders without automatic selection. |
| `scripts/run_retrospective_wavelet_sweep.py` | Retrospective-resolution reconstruction | Calls `wave_retro_lr.pipeline.run_config` once per new lambda, reuses declared completed controls, and records one resumable sweep manifest; the reconstruction backend remains explicit BART `wave -w -f -r <lambda> -g`. |
| `scripts/evaluate_retrospective_wavelet_sweep.py` | Superseded diagnostic | Attempted a separate native-grid reference evaluation; it is not the current retrospective sweep evaluation. Preserve its incomplete output only as diagnostics. |
| `scripts/evaluate_retrospective_wavelet_sweep_matched_grid.py` | Retrospective-resolution evaluation | Evaluation-only reproduction of the accepted matched-grid calculation: linear interpolation to the original 1 mm RAS grid, exact reuse of the hash-bound references and BET brain/edge masks, and per-metric leaders without automatic selection. |
| `scripts/run_retrospective_llr_sweep.py` | Retrospective-resolution reconstruction | Calls the existing retrospective pipeline for each configured corrected split-complex LLR block/lambda setting with mandatory BART GPU, reusable prepared inputs, magnitude/phase export, and resume manifests. |
| `scripts/evaluate_retrospective_llr_sweep_matched_grid.py` | Retrospective-resolution evaluation | Evaluation-only LLR comparison on the accepted matched 1 mm grid with exact original references/masks, FISTA-lambda-zero controls, per-block plots, and no automatic selection. |
...
| `scripts/build_presentation_metrics_csv.py` | Presentation metrics packaging | Combines one or more hash-bound exact-grid tables plus matched-grid retrospective and native descriptive metrics into one presentation-ordered CSV with explicit scope labels. |
| `scripts/export_presentation_orientation_tiffs.py` | Presentation image export | Saves fixed-index or per-orientation center sagittal, coronal, and axial 16-bit TIFFs with manifested slice selection, display scaling, and hashes. |
| `scripts/record_regularization_selection.py` | Selection provenance | Records an explicit user choice and binds it to the evaluated metric table, selected reconstruction, and solver-matched baseline without performing automatic selection. |
